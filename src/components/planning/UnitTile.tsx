...
export default function UnitTile({ unit, selectedUnit, phase }: UnitTileProps) {
  ...
  return (
    <>
      ...
      <div
        className={`${bg} group relative h-24 snap-center justify-items-center rounded-md bg-cover bg-center sm:w-[64px] ${bgColor} ${borderWidth} ${borderColor} p-1 text-center shadow-inner ${hoverBorder}`}
      >
        <div
          className={`h-3 w-[52px] ${healthWidth} ${healthBarColor} absolute bottom-1 rounded-sm transition-all duration-500 ease-out`}
        ></div>
        {/* Popup text */}
