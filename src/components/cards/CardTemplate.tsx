...
export default function CardTemplate({ color, children }: CardTemplateProps) {
  ...
  return (
    ...
    <div
      className={`grid w-52 auto-rows-auto gap-1 rounded-lg border border-white/10 p-1 pt-0 ${bgColor} text-white hover:-translate-y-0.5 hover:shadow-lg hover:shadow-gray-500/50`}
    >
      {children}
    </div>
  );
